...
--w nawiasie (+48)
SELECT CONCAT (right(telefon,1) ,'(+48)',left(telefon,9)) FROM ksiegowosc.pracownicy AS telefon



--b) Zmodyfikuj atrybut telefon w tabeli pracownicy tak, aby numer oddzielony by³ myœlnikami wg
--wzoru: ‘555-222-333’ 
SELECT CONCAT (right(telefon,3)  ,left(telefon,3) , '-', SUBSTRING(telefon,4,3),'-',left(telefon,3)) FROM ksiegowosc.pracownicy

--c)  Wyœwietl dane pracownika, którego nazwisko jest najd³u¿sze, u¿ywaj¹c du¿ych liter
-- gdzies tutaj trzeba uzyc UPPER ???
SELECT TOP 1 * FROM ksiegowosc.pracownicy
WHERE

len(nazwisko) = 
(SELECT max(len(nazwisko)) FROM ksiegowosc.pracownicy);
...
SELECT * FROM ksiegowosc.pracownicy
SELECT *,
HASHBYTES('MD5',(SELECT imie)) as imieMD5
...
SELECT *
FROM
 ksiegowosc.pracownicy pra
 LEFT OUTER JOIN ksiegowosc.pensje pen ON pra.id_pracownika=pen.id_pensji
 INNER JOIN ksiegowosc.premie pre ON pre.kwota=pen.kwota

--g) g) wygeneruj raport (zapytanie), które zwróci w wyniki treœæ wg poni¿szego szablonu:
